...
public class GraphPanel extends JPanel implements MouseListener,MouseMotionListener,KeyListener {
	...
	private Point saveMousePosition;			//Sauvegarde de la derniére position de la souris (Voir MouseDragged)
	...
	public void highlightArc(Graphics2D g2d, Vertex v, Arc arc) {
		...
		//Coordonn�es centrale des deux points en fonction du zoom
		...
		if(arc.getVertex() == v) {	//Arc sur lui m�me
	...
	public void drawEdges(Graphics2D g2d) {
	 ...
	 /**
	   ...
	   * @param d  largeur de la fl�che
	   * @param h  hauteur de la fl�che
	   */
			...
			for(Arc arc : v.getAlArcs()) {					
				...
				if( arc.getVertex() == v ) {	//Arc sur lui m�me
					...
					if(bMirroir && hci.getGraph().isDirected()) {
						/*-----Cacul des points M et N pour placer les deux arcs par rapport � pCenter1*/
						int d = 0;
				        ...
				        xn1 = x;
				        /*-----------------------------------------------------------------------------*/
						/*-----Cacul des points M et N pour placer les deux arcs par rapport � pCenter2*/
				        d = 0;
		...
		if(alSelected.size() != 0 && bDragged==true) {			
			...
			for(String s : hci.hmVertex.keySet()) {
				if (alSelected.contains(s)) {
					...
					centerY = c.getY()+iHeightEdge/2*iZoom;
					if (Math.pow(e.getX() - centerX, 2) + Math.pow(e.getY() - centerY, 2) <= (Math.pow(iWidthEdge/2*iZoom, 2))) {
						/*-----D�placements des sommets s�lectonn�s----*/
						setCursor(new Cursor(Cursor.MOVE_CURSOR));
						//Coordonn�es minimales des points s�lectionn�s
						...
						//Coordonn�es maximales des points s�lectionn�s
						...
						//Calcul de la diff�rence entre la derni�re position de la souris et l'actuelle
						...
						if( (minPosition.x<=0 && deplacement.x>=0 || minPosition.x>0) && 
							...
							for(String edgeSelected : alSelected) {
								...
								//Texte qui affiche les coordonn�es
	...
	public void mouseMoved(MouseEvent e) {
		hci.requestFocus();
		// Permet d'afficher les coordonn�es du point d�s que la souris passe dessus
		Point c = isOnEdge(e);
		if (c!=null) {
	...
	public void keyReleased(KeyEvent e) {
		//CTRL released pour la s�lection
