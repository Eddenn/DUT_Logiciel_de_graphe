...
/**
 * Classe qui fait le lien entre l'IHM et le m�tier et qui v�rifie les actions effectu�es
 * @author Groupe 3
 * @version 2016-01-08
 */
public class Controller implements IControlable, IIhmable {
	...
	public Controller() {
		...
		// Initialisation de l'arrayList qui permet d'impl�menter les fonctions annuler et r�tablir
		initProvSave();
	}
	 ...
	 * -----------------*/
	/**
	 * M�thode qui cr�e un nouveau graphe vide avec les param�tres choisis par l'utilisateur.
	 * @param bOriented true s'il est orient�, false sinon.
	 * @param bValued true s'il est valu�, false sinon.
	 */
	 ...
	 * @param strFileName le chemin o� le fichier doit �tre enregistrer, 
	 * 		  si null, on reprend le chemin sauvegard� pour �craser l'ancienne sauvegarde
	 */
	 ...
	 * @param strFileName le chemin du fichier � charger
	 */
	...
	public void undo() {
		...
		/*if (cptModif > 1) {
			graph = ReaderAdjacencyList.ReadAdjacencyList(new ArrayList<String>(saveVertexList.get(cptModif-2)), hci);
	...
	public void redo() {
		/*if (cptModif >= 0 && cptModif < saveVertexList.size()) {
			graph = ReaderAdjacencyList.ReadAdjacencyList(new ArrayList<String>(saveVertexList.get(cptModif)), hci);
			hci.initHmVertexByTab(saveCoordList.get(cptModif));
			cptModif++;
		}*/
	}
	...
	/**
	 * M�thode qui met � z�ro les arrayList contenant les actions effectu�s depuis la derni�re sauvegarde.
	 */
	...
	/**
	 * M�thode permettant de sauvegarder l'�tat du graphe � une instant t. Utiliser pour sauvegarder les actions effectu�es.
	 */
